...
func (e *EciesHkdfRecipientKem) GenerateKey(kem []byte, hmacAlgo string, hkdfSalt []byte, hkdfInfo []byte, symKeySize int, macKeySize int) ([]byte, error) {
	...
	eciesEphemeralPublicKey := subtle.ImportECDSAPublic(&ephemeralPublicKey)
	eciesPrivateKey := subtle.ImportECDSA(&e.PrivateKey)
	sharedSecret, err := subtle.GenerateShared(eciesEphemeralPublicKey, symKeySize, macKeySize)
	if err != nil {
		return nil, err
	}
